...
class NeuralNetwork {
	...
	// Activation functions (Only sigmoid has a derivative currently)
	...
	derivativeSigmoid(num) {
		return this.sigmoid(num) * (1 - this.sigmoid(num));
	}
		...
		for (let x = (this.layers.length - 1); x > 0; x--) {
			...
			for (let y = 0; y < this.layers[x].length; y++) {
				...
				for (let i = 0; i < this.layers[x - 1].length; i++) {
					...
					// Calulate gradient
					let gradient = this.fakeDSigmoid(this.layers[x][y]);
					gradient *= nodeError;
